...
//
// Model spec (see convert_to_coreml.py). Both engines run the SAME model
// and emit the SAME two tensors, so all post-processing is shared:
//   detection  : Float32 [1, 40, 8400]     -- 4 bbox + 4 class + 32 mask coef
//   prototypes : Float32 [1, 32, 160, 160] -- mask prototypes
// We identify the two outputs by shape (not name), because coremltools
// auto-generates `var_NNNN` names that change every export.
//
// Two interchangeable inference engines, picked per-scan via `engine`:
//   .coreml : Vision + Core ML (PathScout.mlpackage -> .mlmodelc). Vision
//             handles letterbox/scale/colour; runs on ANE/GPU/CPU.
//   .onnx   : native ONNX Runtime (PathScout.onnx), CPU provider. We do the
//             letterbox + RGB/255 + NCHW packing ourselves (the portable
//             path we also hand to Android). Each scan returns `engine` and
//             `latencyMs` so the two can be compared head-to-head.
//
...
// MARK: - Bridge view controller (registers this app-local plugin)
//
// Capacitor 7 only auto-registers plugins that ship as npm packages -- it
// reads capacitor.config.json's `packageClassList` and instantiates those.
// A plugin defined directly in the app target is never in that list, so it
// is never registered and every JS call rejects with "not implemented",
// which surfaces as Path Scout being permanently "unavailable". The fix is
// to register it by hand in a CAPBridgeViewController subclass; the single
// scene in Main.storyboard points at this class instead of the stock one.
class MainViewController: CAPBridgeViewController {
    override func capacitorDidLoad() {
        NSLog("[PathScout] registering plugin instance")
        bridge?.registerPluginInstance(PathScoutPlugin())
    }
}
...
public class PathScoutPlugin: CAPPlugin, CAPBridgedPlugin {
    ...
    @objc func isAvailable(_ call: CAPPluginCall) {
        call.resolve([
            ...
            "engines": [
                "coreml": scanner.coremlAvailable,
                "onnx":   scanner.onnxAvailable,
            ],
        ])
    }
    ...
    @objc func scan(_ call: CAPPluginCall) {
        scanner.wantPreview = call.getBool("preview") ?? false
        // "coreml" (default) runs the model through Vision/Core ML; "onnx"
        // runs the identical model through native ONNX Runtime. Both feed the
        // exact same post-processing, so only inference + the latency differ.
        let engine = PathScoutEngine(rawValue: call.getString("engine") ?? "coreml") ?? .coreml
        scanner.scanOnce(engine: engine) { result in
            switch result {
...
/// Which inference runtime to use for a given scan. Both run the same
/// YOLOv8-seg model and feed the identical post-processing; only the
/// inference call (and the latency it reports) differs.
enum PathScoutEngine: String {
    case coreml   // Vision + Core ML (.mlpackage -> .mlmodelc)
    case onnx     // native ONNX Runtime (PathScout.onnx), CPU provider
}
...
enum PathScoutError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .modelMissing:               return "PathScout model not bundled. Did you drag PathScout.mlpackage / PathScout.onnx into Xcode?"
        case .engineUnavailable(let m):   return "Requested engine unavailable: \(m)"
        ...
        case .preprocessFailed:           return "Failed to letterbox the camera frame for ONNX input."
...
final class PathScoutScanner: NSObject {
    ...
    // `modelAvailable` is true if *either* engine loaded; the per-engine
    // flags let the JS toggle disable an engine that didn't bundle.
    var modelAvailable: Bool { coremlAvailable || onnxAvailable }
    private(set) var coremlAvailable: Bool = false
    private(set) var onnxAvailable: Bool = false
    private(set) var modelURL: URL?
    private var visionModel: VNCoreMLModel?
    private var ortEnv: ORTEnv?
    private var ortSession: ORTSession?

    // Debug preview: when true, scanOnce also composites a segmentation
    // overlay JPEG (see composePreview) into the result dict so the JS side
    // can show a live validation view. Off by default; only set per-call.
    var wantPreview: Bool = false
    private static let ciContext = CIContext(options: nil)

    override init() {
        super.init()
        loadCoreMLIfAvailable()
        loadONNXIfAvailable()
        if !modelAvailable {
            NSLog("[PathScout] no model found in bundle (looked for PathScout.mlmodelc / .mlpackage / PathScout.onnx)")
        }
    }

    private func loadCoreMLIfAvailable() {
        ...
        for name in candidates {
            ...
            if let url = Bundle.main.url(forResource: base, withExtension: ext) {
                ...
                do {
                    ...
                    NSLog("[PathScout] Core ML model loaded from \(url.lastPathComponent)")
                    return
                } catch {
                    NSLog("[PathScout] Failed to load Core ML model at \(url.path): \(error)")
                }
            }
        }
    ...
    private func loadONNXIfAvailable() {
        guard let url = Bundle.main.url(forResource: "PathScout", withExtension: "onnx") else {
            onnxAvailable = false
            return
        }
        do {
            // CPU provider on purpose: this is the portable runtime we also
            // ship to Android, so the latency we report is apples-to-apples
            // with that path rather than secretly re-routing through Core ML.
            ...
            ortEnv = env
            ortSession = session
            if modelURL == nil { modelURL = url }
            onnxAvailable = true
            NSLog("[PathScout] ONNX model loaded from \(url.lastPathComponent)")
        } catch {
            onnxAvailable = false
            NSLog("[PathScout] Failed to load ONNX model at \(url.path): \(error)")
        }
    }
    ...
    /// Grab the latest frame and run it through the requested engine. If the
    /// requested engine isn't bundled we fall back to whichever one is, so a
    /// device that only shipped one model still works.
    func scanOnce(engine: PathScoutEngine,
                  completion: @escaping (Result<[String: Any], Error>) -> Void) {
        let resolved: PathScoutEngine
        switch engine {
        case .coreml: resolved = coremlAvailable ? .coreml : .onnx
        case .onnx:   resolved = onnxAvailable ? .onnx : .coreml
        }
        if resolved == .coreml && !coremlAvailable {
            completion(.failure(PathScoutError.engineUnavailable("coreml"))); return
        }
        if resolved == .onnx && !onnxAvailable {
            completion(.failure(PathScoutError.engineUnavailable("onnx"))); return
        }
        ...
        inferenceQueue.async { [weak self] in
            guard let self = self else { return }
            switch resolved {
            case .coreml: self.scanCoreML(pixelBuffer: pixelBuffer, completion: completion)
            case .onnx:   self.scanONNX(pixelBuffer: pixelBuffer, completion: completion)
            }
        }
    }
    ...
    private func scanCoreML(pixelBuffer: CVPixelBuffer,
                            completion: @escaping (Result<[String: Any], Error>) -> Void) {
        ...
        let request = VNCoreMLRequest(model: vnModel) { req, err in
            if let err = err { reqError = err; return }
            guard let observations = req.results as? [VNCoreMLFeatureValueObservation] else { return }
        ...
        let handler = VNImageRequestHandler(cvPixelBuffer: pixelBuffer,
                                            ...
                                            options: [:])
        do {
            try handler.perform([request])   // synchronous; fills the vars above
        } catch {
            completion(.failure(PathScoutError.inferenceFailed(error.localizedDescription))); return
        }
        if let reqError = reqError {
            completion(.failure(PathScoutError.inferenceFailed(reqError.localizedDescription))); return
        }
        ...
        // det / proto stay retained for the duration of this call, so the
        // raw float pointers we hand to buildGuidance remain valid.
        var dict = buildGuidance(detection: bindFloatPointer(det),
                                 prototypes: bindFloatPointer(proto),
                                 pixelBuffer: pixelBuffer)
        dict["engine"] = "coreml"
        dict["latencyMs"] = latencyMs
        completion(.success(dict))
    }

    // ---- ONNX Runtime inference -----------------------------------------

    private func scanONNX(pixelBuffer: CVPixelBuffer,
                          completion: @escaping (Result<[String: Any], Error>) -> Void) {
        guard let session = ortSession else {
            completion(.failure(PathScoutError.engineUnavailable("onnx"))); return
        }
        let size = Int(Self.modelInputSize)
        guard var chw = makeLetterboxedCHW(pixelBuffer: pixelBuffer, size: size) else {
            completion(.failure(PathScoutError.preprocessFailed)); return
        }
        do {
            let t0 = CFAbsoluteTimeGetCurrent()
            let data = NSMutableData(bytes: &chw, length: chw.count * MemoryLayout<Float>.stride)
            ...
            let outputs = try session.run(withInputs: [inputName: inputValue],
                                          outputNames: Set(outNames),
                                          runOptions: nil)
            // Identify outputs by shape, same convention as the Core ML path.
            var detVal: ORTValue?
            var protoVal: ORTValue?
            for (_, v) in outputs {
                let shp = try v.tensorTypeAndShapeInfo().shape.map { $0.intValue }
                if shp.count == 3 && shp[1] == Self.detectionChannels { detVal = v }
                else if shp.count == 4 && shp[1] == Self.maskProtoChannels { protoVal = v }
            }
            guard let detV = detVal, let protoV = protoVal else {
                completion(.failure(PathScoutError.malformedOutput)); return
            }
            let detData = try detV.tensorData() as Data
            let protoData = try protoV.tensorData() as Data
            let latencyMs = (CFAbsoluteTimeGetCurrent() - t0) * 1000.0

            var dict: [String: Any] = [:]
            detData.withUnsafeBytes { (detRaw: UnsafeRawBufferPointer) in
                protoData.withUnsafeBytes { (protoRaw: UnsafeRawBufferPointer) in
                    guard let detPtr = detRaw.baseAddress?.assumingMemoryBound(to: Float.self),
                          let protoPtr = protoRaw.baseAddress?.assumingMemoryBound(to: Float.self)
                    else { return }
                    dict = self.buildGuidance(detection: detPtr, prototypes: protoPtr,
                                              pixelBuffer: pixelBuffer)
                }
            }
            if dict.isEmpty {
    ...
    // ---- ONNX input preprocessing ---------------------------------------

    /// Letterbox the camera frame into a `size`x`size` RGB tensor in NCHW
    /// order, normalised to 0..1 -- exactly the input the YOLOv8-seg ONNX
    /// export expects. We replicate Vision's `.scaleFit` (aspect-preserving,
    /// centred, black padding) and a vertical flip so row 0 == top of frame,
    /// matching the orientation Core ML receives via `.up`. Both engines then
    /// produce masks in the same coordinate frame and share post-processing.
    private func makeLetterboxedCHW(pixelBuffer: CVPixelBuffer, size: Int) -> [Float]? {
        ...
        let ox = (CGFloat(size) - dw) / 2, oy = (CGFloat(size) - dh) / 2
        ctx.draw(cg, in: CGRect(x: ox, y: oy, width: dw, height: dh))

        // CGContext is bottom-left origin, so rgba row 0 is the bottom of the
        // image; flip vertically while packing CHW so row 0 == top of frame.
        let plane = size * size
        var out = [Float](repeating: 0, count: 3 * plane)
        let inv: Float = 1.0 / 255.0
        for y in 0..<size {
            let srcRow = (size - 1 - y) * bytesPerRow
            let dstRow = y * size
            for x in 0..<size {
                let p = srcRow + x * 4
                out[dstRow + x]             = Float(rgba[p])     * inv   // R
                out[plane + dstRow + x]     = Float(rgba[p + 1]) * inv   // G
                out[2 * plane + dstRow + x] = Float(rgba[p + 2]) * inv   // B
            }
        }
        return out
    }
    ...
    private func buildGuidance(detection detPtr: UnsafePointer<Float>,
                               prototypes protoPtr: UnsafePointer<Float>,
                               pixelBuffer: CVPixelBuffer) -> [String: Any] {

        // 1) Decode detection tensor → list of (classIdx, score, box, mask coefs)
        let detections = decodeDetections(detPtr)
        ...
        // Debug overlay: only composited when explicitly requested, since it
        // costs an extra image encode per frame.
        if wantPreview,
           let jpeg = composePreview(pixelBuffer: pixelBuffer, classMaskUnion: classMaskUnion) {
            dict["previewJpeg"] = jpeg
        }

        return dict
    }

    // ---- Debug preview compositing --------------------------------------

    /// Composite the model's per-class segmentation masks as translucent
    /// colored regions over the camera frame, returning a base64 JPEG. The
    /// frame is letterboxed into a square exactly like the model's `.scaleFit`
    /// input so the 160x160 masks line up with what the model actually saw.
    private func composePreview(pixelBuffer: CVPixelBuffer,
                                classMaskUnion: [[UInt8]]) -> String? {
        let target: CGFloat = 320
        let maskSize = Self.maskProtoSize

        // Base camera image (raw buffer, same orientation the model received).
        let ci = CIImage(cvPixelBuffer: pixelBuffer)
        guard let baseCG = Self.ciContext.createCGImage(ci, from: ci.extent) else { return nil }

        // Build a straight-alpha RGBA overlay at mask resolution. Premultiply
        // so it draws correctly through CoreGraphics. Priority keeps curbs
        // visible where classes overlap: curb_down > curb_up > sidewalk > road.
        let colors: [(UInt8, UInt8, UInt8)] = [
            (255, 59, 48),    // 0 curb_down  (red)
            (255, 149, 0),    // 1 curb_up    (orange)
            (0, 122, 255),    // 2 road       (blue)
            (52, 199, 89),    // 3 sidewalk   (green)
        ]
        let priority = [0, 1, 3, 2]
        let alpha: UInt8 = 140
        let af = Float(alpha) / 255.0
        let bpp = 4
        let bytesPerRow = maskSize * bpp
        var rgba = [UInt8](repeating: 0, count: maskSize * maskSize * bpp)
        for pi in 0..<(maskSize * maskSize) {
            var chosen = -1
            for c in priority where classMaskUnion[c][pi] != 0 { chosen = c; break }
            if chosen < 0 { continue }
            let col = colors[chosen]
            let o = pi * bpp
            rgba[o]     = UInt8(Float(col.0) * af)
            rgba[o + 1] = UInt8(Float(col.1) * af)
            rgba[o + 2] = UInt8(Float(col.2) * af)
            rgba[o + 3] = alpha
        }
        let space = CGColorSpaceCreateDeviceRGB()
        ...
        // Letterbox the base frame into the square; the mask overlay covers
        // the full square (it includes the letterbox bars, as the model saw).
        let imgW = CGFloat(baseCG.width), imgH = CGFloat(baseCG.height)
        let scale = min(target / imgW, target / imgH)
        let drawW = imgW * scale, drawH = imgH * scale
        let ox = (target - drawW) / 2, oy = (target - drawH) / 2

        let renderer = UIGraphicsImageRenderer(size: CGSize(width: target, height: target))
        let out = renderer.image { rctx in
            UIColor.black.setFill()
            rctx.fill(CGRect(x: 0, y: 0, width: target, height: target))
            UIImage(cgImage: baseCG).draw(in: CGRect(x: ox, y: oy, width: drawW, height: drawH))
            rctx.cgContext.interpolationQuality = .none
            UIImage(cgImage: overlayCG).draw(in: CGRect(x: 0, y: 0, width: target, height: target))
        }
        return out.jpegData(compressionQuality: 0.5)?.base64EncodedString()
    }
    ...
    /// index = channel * 8400 + anchor. Both engines hand us the identical
    /// `[1, 40, 8400]` buffer, so this is shared verbatim.
    private func decodeDetections(_ p: UnsafePointer<Float>) -> [DecodedDet] {
